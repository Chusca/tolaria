...
3. **No orphan state updates**: Never call `updateEntry()` before the corresponding `handleUpdateFrontmatter()` or `handleDeleteProperty()` has resolved. Type metadata actions in `useEntryActions` follow this rule — create the missing type document if needed, write frontmatter first, then update state. If missing type creation collides with an existing note filename, the action stops after the existing creation toast instead of applying orphaned state. A Type's `_filename_template` resolves `{{date}}`, `{{time}}`, `{{title}}`, and `{{type}}` tokens at note-creation time to produce date-based filenames, and a resolved path that already exists opens the existing note instead of creating a duplicate.
